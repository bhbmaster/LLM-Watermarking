 ...
 *
 * ── The bracket ────────────────────────────────────────────────────────────────────
 * Do not change logits. Sample several candidate tokens from the model distribution p.
 * Then run a knock-out tournament:
 *
 *   • For each layer ℓ = 1..m, the key and context seed a g-function.
 *         g_ℓ(token) is 0 or 1 (a keyed coin per token per layer).
 *   • Draw 2^m candidates independently from p, with the same distribution.
 *   • Layer 1: pair the candidates. The token with the larger g_1 wins.
 *     If the g values are equal, pick one at random.
 *   • Layer 2: pair the winners and compare g_2. Repeat until one token remains.
 *
 * Every candidate came from p. The output is p re-weighted toward lucky g-values.
 * A detector recomputes those g-values. Watermarked text has a mean above 0.5.
 * Chance is 0.5. See `detect.ts`.
 *
 * Why is this "distortion-free"? Average over the random key.
 * Each token is then emitted with exactly p(token).
 * The key only chooses which tokens get a boost. The boosts cancel in expectation.
 * Kirchenbauer soft lists are not distortion-free.
 * Those lists favour green tokens for every key.
 *
 * ── Efficient equivalent: one layer at a time in closed form ───────────────────────
 * You cannot simulate 2^30 candidates. You do not need to.
 * With independent candidates, one layer maps p to p' with a closed formula.
 * Let q = Σ_x p(x)·g(x) be the probability mass of tokens with g=1.
 * Draw a pair (X, Y) from p. X wins if g(X) > g(Y).
 * If g(X) = g(Y), X wins with probability 1/2. So
 *
 ...
 *
 * Check: Σ = q(2-q) + (1-q)(1-q) = 1.
 * Winners of layer 1 are independent draws from p'.
 * Layer 2 applies the same formula to p' with g_2. Repeat for m layers.
 * Then draw one token from p_m. That is the tournament winner distribution.
 * The Hugging Face `SynthIDTextWatermarkLogitsProcessor` uses this form.
 * `tournamentBracketSample` is the literal bracket. Use it to test `applyTournament`.
 */
...
export function applyTournament(dist: Distribution, seed: number, depth: number): void {
  ...
  if (n <= 1) return; // nothing to choose between - the watermark cannot embed anything here
