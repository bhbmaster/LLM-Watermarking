 ...
 *
 * We implement this here because Transformers.js v4 does not ship top-k / top-p, and
 ...
 * `-Infinity` logits (e.g. hard-red-listed tokens) become exactly 0 probability.
 */
 ...
 * Uses a size-k min-heap: O(V log k) instead of sorting the whole vocabulary (V ≈ 150k).
 * The heap root is always the *smallest* kept value, so a new value only enters if it
 ...
 * so high-entropy tokens are where the green list / tournament actually bite.
 */
